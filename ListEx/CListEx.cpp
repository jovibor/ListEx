...
LISTEX::IListExPtr LISTEX::CreateListEx() {
	return IListExPtr { new LISTEX::INTERNAL::CListEx() };
}
...
bool CListEx::Create(const LISTEXCREATE& lcs)
{
	...
	//Cell Tooltip.
	if (!m_wndCellTT.CreateEx(WS_EX_TOPMOST, TOOLTIPS_CLASS, nullptr, lcs.dwTTStyleCell,
	...
	//Link Tooltip.
	if (!m_wndLinkTT.CreateEx(WS_EX_TOPMOST, TOOLTIPS_CLASS, nullptr, lcs.dwTTStyleLink,
		CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, nullptr, nullptr)) {
		return false;
	}
	...
	if (m_fHighLatency) { //Tooltip for HighLatency mode.
		if (!m_wndRowTT.CreateEx(WS_EX_TOPMOST, TOOLTIPS_CLASS, nullptr,
